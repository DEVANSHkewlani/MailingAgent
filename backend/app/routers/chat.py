"""
Chat Router — conversation management and LLM-powered agent messaging.
...
Security:
  ...
  - Instruction input is length-limited to prevent cost explosion
"""
...
from app.permissions.rate_limit import enforce_rate_limit
...
async def get_messages(
    ...
    if conv and str(conv["user_id"]) != current_user["user_id"]:
...
async def get_email_body(
    ...
    try:
        ...
        return {"body": "Invalid email ID format."}
        ...
        if pending_actions:
            ...
            try:
                ...
                if classification == "APPROVE":
                    ...
                    # EDIT or NEW: Mark previous actions as rejected
                    for action in pending_actions:
                        approval_id = action.get("approval_db_id")
                        if approval_id:
                            await db.execute(
    ...
    if pending:
        ...
        if not has_groq:
            ...
            for action in pending:
                ...
                if action["type"] == "send_email":
                    lines.append(f"- Send the draft reply to {action_payload.get('to', 'the recipient')} about \"{action_payload.get('subject', 'the thread')}\".")
                elif action["type"] == "create_event":
                    lines.append(f"- Create \"{action_payload.get('title', 'Calendar event')}\" from {action_payload.get('start_iso', 'the requested start')} to {action_payload.get('end_iso', 'the requested end')}.")
                else:
